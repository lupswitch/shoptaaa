 	...
 	class MyPayPal {
		...
		function SetExpressCheckout($products, $charges, $noshipping='1'){
			...
			foreach($products as $p => $item){
				...
				$padata .=	'&L_PAYMENTREQUEST_0_DESC'.$p.'=abc';
		...
		function DoExpressCheckoutPayment(){
			...
			if(!empty(_SESSION('ppl_products'))&&!empty(_SESSION('ppl_charges'))){
				...
				if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
					...
					$tranID = urldecode($httpParsedResponseAr["PAYMENTINFO_0_TRANSACTIONID"]);
					...
					*/
					...
					if('Completed' == $httpParsedResponseAr["PAYMENTINFO_0_PAYMENTSTATUS"]){
						...
						$url = $_SERVER;
						//print_r($url);
						//echo base_url("front/paypalexpress/PaypalController/");
						/* echo "<pre>";
						print_r($httpParsedResponseAr);
						echo "</pre>"; */
						
						$ammount =  str_replace("%2e",".",$httpParsedResponseAr['PAYMENTINFO_0_AMT']);
						
						
						
						
						
						//redirect(base_url("front/paypalexpress/PaypalController/sucess_treansection?amt=".$ammount));
					}
					...
					/* echo "<pre>";
						...
						$ammount =  str_replace("%2e",".",$httpParsedResponseAr['PAYMENTINFO_0_AMT']);
						$msg = 'Transaction Complete, but payment may still be pending! '.
						'If that\'s the case, You can manually authorize this payment in your <a target="_new" href="http://www.paypal.com">Paypal Account';
					}
				...
				//pr($this->GetTransactionDetails());
				
				return	$rtnPaymentData = array(
											'Paypal_trans_id' => $tranID,
											'ammount'	=>	$ammount,
											'msg'		=> $msg,
									); 
		...
		function GetTransactionDetails(){
			...
			if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])){
				...
				$count = count(array_keys($httpParsedResponseAr));
				$numberofproducts = $_GET["number_of_val"]-1;
				
				$transactionid = $httpParsedResponseAr['PAYMENTREQUESTINFO_0_TRANSACTIONID'];
				$ammount = str_replace("%2e",".",$httpParsedResponseAr['AMT']);
				
				/***website user information start***/
		...
		function PPHttpPost($methodName_, $nvpStr_) {
			...
			curl_setopt($ch, CURLOPT_SSLVERSION , 1); /* add on date 6/7/2016 due to error.  */
