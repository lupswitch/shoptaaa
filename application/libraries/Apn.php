	...
	class Apn
	{
		...
		*/
		...
		/**
			...
			* @return <string>
		*/
		protected function generatePayload($message, $badge = NULL, $sound = NULL, $newstand = false,$ids) 
		{
			$body = array();
			// additional data
			...
			//message
			$body['aps'] = array('alert' => $message,$ids);
			//badge
			if ($badge)
		...
		*/
		protected function sendPayloadEnhance($deviceToken, $payload, $expiry = 86400) 
		{
			...
			. $payload;
			$request_unpacked = @unpack('Ccommand/Nidentifier/Nexpiry/ntoken_length/H64device_token/npayload_length/A*payload', $request); 
				// payload
		...
		*/
		...
		/**
			...
			* @param <string> $sound
		*/
		public function sendMessage($deviceToken, $message, $badge = NULL, $sound = NULL, $ids=array(), $expiry = '', $newstand = false)
		{ 
			...
			$payload = $this->generatePayload($message, $badge, $sound, $newstand,$ids);
